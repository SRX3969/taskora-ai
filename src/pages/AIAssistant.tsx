 ...
 export default function AIAssistant() {
  ...
  const isMobile = useIsMobile();
   ...
   useEffect(() => {
     const loadMessages = async () => {
       if (currentConversationId) {
         try {
           const dbMessages = await fetchMessages(currentConversationId);
   ...
   const startNewChat = async () => {
     try {
       const newConvo = await createConversation.mutateAsync("New Chat");
   ...
   const handleDeleteConversation = async (id: string, e: React.MouseEvent) => {
     ...
     try {
       ...
       if (currentConversationId === id) {
         setCurrentConversationId(conversations.filter(c => c.id !== id)[0]?.id || null);
       }
     } catch (err) {
   ...
   const sendMessage = async (text: string) => {
     ...
     if (!conversationId) {
       try {
         ...
         const newConvo = await createConversation.mutateAsync(title);
       ...
       const title = text.slice(0, 50) + (text.length > 50 ? "..." : "");
       updateConversationTitle.mutate({ id: conversationId, title });
     }
     ...
     try {
       await addMessage.mutateAsync({ conversationId, role: "user", content: text });
     } catch (err) {
     ...
     await streamChat({
       ...
       onDone: async () => {
         ...
         if (assistantSoFar && conversationId) {
           try {
             await addMessage.mutateAsync({ conversationId, role: "assistant", content: assistantSoFar });
           } catch (err) {
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
              {messages.length === 0 && !isTyping ? (
                 <div className="flex flex-col items-center justify-center h-full text-center">
                   ...
                   <h2 className="text-xl font-semibold mb-2">Hello! I'm Proddy</h2>
                   <p className="text-muted-foreground max-w-md">
                     Your AI productivity assistant. I can help you with tasks, scheduling, note-taking, and much more. What would you like to know?
                   </p>
                 </div>
               ) : (
